...
export default function Signals() {
  ...
  return <div className="safe-shell mx-auto max-w-6xl pb-24 pt-6 md:pb-12"><div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><span className="z-pill"><SlidersHorizontal className="mr-1 h-3.5 w-3.5" />Feed structuré</span><h1 className="mt-3 font-display text-4xl font-bold md:text-5xl">Signals</h1><p className="mt-2 max-w-2xl text-muted-foreground">Questions d’actualité filtrées par tension, région et catégorie.</p></div>{feedback && <div className="z-glass max-w-sm border-primary/30 p-3 text-sm font-semibold text-primary">{feedback}</div>}</div><div className="sticky top-16 z-30 mb-5 space-y-3 border-y border-line bg-background/88 py-3 backdrop-blur-xl md:rounded-lg md:border md:px-3"><div className="z-no-scrollbar flex gap-2 overflow-x-auto pb-1">{feedFilters.map((f) => <Button key={f} variant={feed === f ? "default" : "outline"} size="sm" onClick={() => setFeed(f)}>{f}</Button>)}</div><div className="z-no-scrollbar flex gap-2 overflow-x-auto pb-1"><Button variant={region === "Toutes" ? "default" : "outline"} size="sm" onClick={() => setRegion("Toutes")}>Toutes régions</Button>{regions.map((r) => <Button key={r} variant={region === r ? "default" : "outline"} size="sm" onClick={() => setRegion(r)}>{r}</Button>)}</div><div className="z-no-scrollbar flex gap-2 overflow-x-auto pb-1"><Button variant={category === "Toutes" ? "default" : "outline"} size="sm" onClick={() => setCategory("Toutes")}>Toutes catégories</Button>{categories.map((c) => <Button key={c} variant={category === c ? "default" : "outline"} size="sm" onClick={() => setCategory(c)}>{c}</Button>)}</div></div>{filtered.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((signal) => <SignalCard key={signal.id} signal={signal} onVote={vote} />)}</div> : <div className="z-card p-8 text-center"><h2 className="font-display text-2xl font-bold">Aucun signal</h2><p className="mt-2 text-muted-foreground">Essayez un filtre moins strict.</p></div>}</div>;
}
